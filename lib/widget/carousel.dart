...
class Carousel extends StatefulWidget {
  ...
  Carousel(
      ...
      : _length = children.length,
        ...
        assert((initIndex >= 0) && (initIndex < children.length), 'initIndex 越界');
...
class CarouselState extends State<Carousel> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (widget.autoPlay) {
      ...
      timer = Timer.periodic(playInterval, (Timer t) {
        ...
        _pageController.animateToPage(toPage, duration: playDuration, curve: widget.curve);
      });
    }
  }
  ...
  Widget _buildIndicators() {
    ...
    for (int i = 0; i < widget._length; i++) {
      Color color = _realCurrentPage == i ? Theme.of(context).primaryColor : Colors.white;
