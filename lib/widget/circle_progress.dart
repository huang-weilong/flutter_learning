...
class _CircleProgressState extends State<CircleProgress> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _controller = AnimationController(duration: Duration(seconds: 1), upperBound: widget.progress, vsync: this);
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      painter: _CircleProgressPainter(_controller.value, widget.primaryColor, widget.secondaryColor, widget.strokeWidth),
      child: Center(
        child: Text(
...
class _CircleProgressPainter extends CustomPainter {
  ...
  _CircleProgressPainter(this.progress, this.primaryColor, this.secondaryColor, this.strokeWidth) : assert(progress >= 0 && progress <= 1);
